...
class Auboi5Robot:
    ...
    def move_to_target_in_cartesian(self, pos, rpy_xyz):
        ...
        * NOTES:
        ...
        if self.rshd >= 0 and self.connected:
            ...
            # print("r2==", rpy_xyz)
            # print("r2-type==", type(rpy_xyz))
            # print("r2-len==", len(rpy_xyz))

            # for i in range(len(rpy_xyz), -1, -1):
            #     print(i)

            # rpy_xyz = [rpy_xyz[i] for i in range(len(rpy_xyz), -1, -1)]
            # rpy_xyz.reverse()
            # print("r3==", rpy_xyz)
            # print("r3-type==", type(rpy_xyz))
            ...
            # print("join==", joint_radian)
            # print("pos==", pos)
            # print("pos-type==", type(pos))

            # print("2ing========================================")

            # 这句报错  调用的dll   不知道原因  12-6
    ...
    # def ZB_to_joint(self, pos, ori):
    #     """
    #     * FUNCTION:    ZB_to_joint
    #     * DESCRIPTION: 给出笛卡尔坐标值和欧拉角，机械臂轴动到目标位置和姿态
    #     * INPUTS:      pos:位置坐标（x，y，z），单位(m)
    #     *              rpy：欧拉角（rx，ry，rz）,单位（度）
    #     * OUTPUTS:
