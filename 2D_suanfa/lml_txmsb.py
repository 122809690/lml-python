...
def tupian_bianhuan(img):
    ...
    # print(frame_mono[1])
    # img[:int(len(img)/2-60), :int(len(img[0])/2)] *= 0.5
    ...
    img[:int(len(img) / 2), :int(len(img[0]) / 2)] *= 0.5
    img[int(len(img) / 2):, int(len(img[0]) / 2):] *= 2
    # cv.imshow("2", img)

    img = cv2.blur(img, (4, 4))  # 均值处理 即模糊
    ...
    # clip(0, 255)剪切上下限 避免运算后像素值超过255造成过曝异常
    # astype(np.uint8) 将图片像素的float64数据转换成uint8 不然imshow显示会异常 （虽然对识别程序来说没影响
    # print(type(img[0][0]))
